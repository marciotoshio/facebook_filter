...
FacebookFilter.doTheJob = function (story) {
	...
	var contentContainer = story.querySelector('.mainWrapper');
	var containerText = FacebookFilter.getContainerText(contentContainer);
	if(containerText) {
		for(var i = 0; i < filterKeys.length; i++) {
			if(filterKeys[i] && containerText.ff_contains(filterKeys[i])) {
...
FacebookFilter.getContainerText = function(container) {
