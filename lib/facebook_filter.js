...
FacebookFilter.byMessageBody = function (story) {
	...
	if(contentContainer != null) {
		for(var i = 0; i < filterKeys.length; i++) {
			if(filterKeys[i] && contentContainer.innerText.containsExactly(filterKeys[i])) {
				FacebookFilter.filter(story)
			}
		}
	}
...
FacebookFilter.byAttachment = function (story) {
	...
	if(contentContainerTitle != null && contentContainerDesc != null ) {
		for(var i = 0; i < filterKeys.length; i++) {
			if(filterKeys[i] && (contentContainerTitle.innerText.containsExactly(filterKeys[i]) || contentContainerDesc.innerText.containsExactly(filterKeys[i]))) {
				FacebookFilter.filter(story)
			}
		}
	}
