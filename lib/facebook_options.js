...
FacebookFilter.Options = function() {
	...
	this.checkOldOptions = function() {
		...
		if(oldOptions.messageBodyKeys) {
			concatKeys(newOptions, oldOptions.messageBodyKeys);
		}
		if(oldOptions.attachmentKeys) {
			concatKeys(newOptions, oldOptions.attachmentKeys);
		}
		if(oldOptions.authorKeys) {
			concatKeys(newOptions, oldOptions.authorKeys);
		}
		function concatKeys(newOptions, oldKeys) {
			newOptions.keys = newOptions.keys.concat(oldKeys);
			save(newOptions);
		}
	}
